// uniffi scaffolding generated from src/eightviews.udl
uniffi::include_scaffolding!("eightviews");
...
// ── Archive namespace functions (UDL `namespace eightviews`) ─────────────────
...
/// - macOS: ~/Library/Application Support/8views/sessions.sqlite
/// - Linux: ~/.local/share/8views/sessions.sqlite
/// - Fallback: $TMPDIR/8views_sessions.sqlite
#[cfg(not(test))]
...
/// Open the OCR cache (same SQLite file as the session DB).
#[cfg(not(test))]
fn open_ocr_db() -> Result<sc_storage::OcrCache, ScError> {
    let base = dirs::data_dir()
        .or_else(|| dirs::home_dir().map(|h| h.join(".local/share")))
        .unwrap_or_else(std::env::temp_dir);
    let app_dir = base.join("8views");
    ...
    let db_path = app_dir.join("sessions.sqlite");
    sc_storage::OcrCache::open(&db_path).map_err(|_| ScError::Storage)
}

#[cfg(test)]
fn open_ocr_db() -> Result<sc_storage::OcrCache, ScError> {
    sc_storage::OcrCache::open_in_memory().map_err(|_| ScError::Storage)
}

/// C-callable: store Vision OCR text for `(archive_path, page_index)`.
///
/// `text` must be a valid NUL-terminated UTF-8 C string.
/// No-op if `archive_path` or `text` is NULL.
///
/// # Safety
/// All pointer arguments must be valid NUL-terminated UTF-8 C strings or NULL.
#[no_mangle]
pub unsafe extern "C" fn sc_ocr_store(
    archive_path: *const std::ffi::c_char,
    page_index: u32,
    text: *const std::ffi::c_char,
    archive_mtime_secs: i64,
) {
    if archive_path.is_null() || text.is_null() {
        return;
    }
    let path_str = match std::ffi::CStr::from_ptr(archive_path).to_str() {
        Ok(s) => s,
    ...
    if let Ok(cache) = open_ocr_db() {
        let _ = cache.store(path_str, page_index, text_str, archive_mtime_secs);
    }
}

/// C-callable: return `true` if a VALID cache entry exists for `(archive_path, page_index)`.
///
/// The entry is valid only if its stored `archive_mtime` matches `current_mtime_secs`.
/// Prefer this over `sc_ocr_has` to avoid returning stale OCR text after the archive
/// file is replaced or modified.
///
/// # Safety
/// `archive_path` must be a valid NUL-terminated UTF-8 C string or NULL.
#[no_mangle]
pub unsafe extern "C" fn sc_ocr_has_valid(
    archive_path: *const std::ffi::c_char,
    page_index: u32,
    ...
    open_ocr_db()
        .map(|cache| cache.has_valid(path_str, page_index, current_mtime_secs))
        .unwrap_or(false)
}

/// C-callable: return `true` if OCR text is cached for `(archive_path, page_index)`.
/// Does not validate mtime — prefer `sc_ocr_has_valid` for correctness.
///
/// # Safety
/// `archive_path` must be a valid NUL-terminated UTF-8 C string or NULL.
#[no_mangle]
pub unsafe extern "C" fn sc_ocr_has(
    archive_path: *const std::ffi::c_char,
    page_index: u32,
) -> bool {
    if archive_path.is_null() {
    ...
    open_ocr_db()
        .map(|cache| cache.has(path_str, page_index))
        .unwrap_or(false)
}

/// C-callable: retrieve cached OCR text for `(archive_path, page_index)`.
///
/// Returns a heap-allocated byte buffer of the UTF-8 text, writing the byte
/// count (excluding NUL) to `*out_len`.  Caller must release with
/// `sc_free_bytes(ptr, *out_len + 1)` (the buffer includes a trailing NUL byte,
/// so the total allocation is `*out_len + 1`).
///
/// Returns NULL if there is no cache entry or on error.
///
/// # Safety
/// `archive_path` must be a valid NUL-terminated UTF-8 C string.
/// `out_len` must be a valid non-null pointer.
#[no_mangle]
pub unsafe extern "C" fn sc_ocr_get(
    archive_path: *const std::ffi::c_char,
    page_index: u32,
    out_len: *mut usize,
) -> *mut u8 {
    ...
    let text = match open_ocr_db().ok().and_then(|c| c.get(path_str, page_index)) {
        Some(t) => t,
        None => return std::ptr::null_mut(),
    };
    // Return a NUL-terminated buffer so callers can use [NSString stringWithUTF8String:].
...
/// C-callable: remove all cached OCR text for `archive_path`.
///
/// Call when a session ends or the archive is removed from the session list.
/// No-op if `archive_path` is NULL.
///
/// # Safety
/// `archive_path` must be a valid NUL-terminated UTF-8 C string or NULL.
#[no_mangle]
pub unsafe extern "C" fn sc_ocr_clear(archive_path: *const std::ffi::c_char) {
    if archive_path.is_null() {
        return;
    }
    let path_str = match std::ffi::CStr::from_ptr(archive_path).to_str() {
...
///
/// C-callable: return true if a session record exists for `archive_path`.
///
/// Cheaper than `sc_session_load` — does not deserialise state.
/// Use this during migration to avoid overwriting existing records.
///
/// # Safety
/// `archive_path` must be a valid NUL-terminated UTF-8 C string.
#[no_mangle]
pub unsafe extern "C" fn sc_session_exists(archive_path: *const std::ffi::c_char) -> bool {
    if archive_path.is_null() {
        return false;
    }
    let path_str = match std::ffi::CStr::from_ptr(archive_path).to_str() {
...
mod tests {
    ...
    // ── sc_session_exists logic tests ────────────────────────────────────────
    // Note: in cfg(test), open_session_db() returns a new in-memory DB each call,
    // so C-FFI save/exists chaining would always see an empty DB.
    // We test the underlying SessionManager directly instead.

    #[test]
    fn session_exists_false_before_save() {
        let mgr = sc_storage::SessionManager::open_in_memory().unwrap();
        assert!(!mgr.exists("/sc_test_migration.cbz"));
    }

    #[test]
    fn session_exists_true_after_save() {
        let mgr = sc_storage::SessionManager::open_in_memory().unwrap();
        let path = "/sc_test_migration.cbz";
        assert!(!mgr.exists(path));
        mgr.save(path, &sc_storage::SessionState::default()).unwrap();
        assert!(mgr.exists(path));
    }

    #[test]
    fn session_exists_null_path_c_returns_false() {
        let result = unsafe { sc_session_exists(std::ptr::null()) };
        assert!(!result, "null path must return false");
    }
}
