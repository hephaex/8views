...
//! exposed by `eightviews` (the `sc-ffi` library crate, whose lib name is
//! `eightviews`), `sc_image`, and `sc_archive`.
//!
//! # Fixture
//!
...
fn archive_to_image_load() {
    ...
    let pages = eightviews::archive_list_pages(path).expect("archive_list_pages failed");
    assert_eq!(pages.len(), 3, "expected 3 pages, got {}", pages.len());

    // Read the first page and decode it.
    let bytes = eightviews::archive_read_page(path, 0).expect("archive_read_page(0) failed");
...
fn archive_read_first_and_scale() {
    ...
    let bytes =
        eightviews::archive_read_first_image(path).expect("archive_read_first_image failed");
...
fn archive_to_thumbnails_parallel() {
    ...
    let entries: Vec<(usize, Vec<u8>)> = pages
        .iter()
        .map(|p| {
            let bytes = eightviews::archive_read_page(path, p.index)
...
fn archive_to_thumbnail_via_ffi() {
    ...
    for i in 0..3u32 {
        let bytes = eightviews::archive_read_page(path, i)
