...
CREATE INDEX IF NOT EXISTS idx_ocr_cache_archive ON ocr_cache(archive_path);
";

/// Add archive_mtime column if missing (idempotent via IGNORE on constraint error).
const SCHEMA_V4: &str = "
