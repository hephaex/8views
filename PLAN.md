...
- Cargo workspace 생성 (`simple-comic-core/`)
...
- `SimpleComicAppDelegate.m` — 아카이브 오픈 Rust 코어 위임
...
- [ ] RAR/CBR 아카이브 읽기
- [ ] 7z 아카이브 읽기
- [ ] TAR/TGZ/TBZ2 아카이브 읽기
- [ ] 폴더(이미지 디렉토리) 열기
- [ ] PDF 페이지 표시
- [ ] 두 페이지 펼치기(Two-Page Spread)
- [ ] 페이지 순서 반전 (우→좌 망가 모드)
- [ ] 줌/회전 (세션별 저장)
- [ ] 세션 복원 (앱 재시작 후 마지막 위치)
- [ ] 썸네일 뷰
- [ ] OCR 텍스트 선택 + 검색
- [ ] Touch Bar 스크러버
- [ ] QuickLook 썸네일/미리보기
...
| 지표 | 목표 | 측정 방법 |
|------|------|---------|
| 아카이브 오픈 (200페이지 CBZ) | < 500ms | Bench |
| 페이지 전환 | < 50ms | UI 프레임 타임 |
| 썸네일 생성 (200페이지, 병렬) | < 3s | Bench |
| 메모리 (200페이지 CBZ 열기) | < 200MB | Instruments |
| QuickLook 썸네일 생성 | < 1s | Bench |

### 코드 품질
- [ ] `cargo test` 통과 (Rust 코어 전체)
